...
- **已分析**：内部机制改进路线已分层为目标 logit 对齐 → AttnLRP 候选定位 → Top-K Activation Patching 因果验证；方案一已形成“多 token 序列 log-prob margin → Gradient×Input shadow MVP → IG → predicted-target 部署模式”的详细蓝图，尚未实施代码改动
