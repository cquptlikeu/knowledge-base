...
- 技术选型或概念查询 → 先搜索 `$KB/wiki/`（先读 `_index.md` 定位，再定向读取），搜索不到或者搜索结果不够好（自行做判断）时，就必须去联网搜索，务必保证搜索结果的质量。
- 遇到值得保存的文章/文档 → 保存到 `$KB/raw/definitely/`（明确有价值）或 `casually/`（随手存），询问用户确定具体保存位置，会话结束时告知用户可采集
